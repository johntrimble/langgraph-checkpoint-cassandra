...
class CassandraSaver(BaseCheckpointSaver):
    ...
    async def asetup(self, replication_factor: int = 3) -> None:
        """
        Set up the checkpoint database schema asynchronously.

        This method creates the necessary tables using migrations.
        It MUST be called before using the checkpointer with async operations.
        ...
        # Apply any pending migrations (sync operation)
        ...
        # Setup queryable metadata columns and indexes (async)
        if self.queryable_metadata:
            await self._asetup_queryable_metadata()

        # Prepare statements (sync - prepare is always sync in cassandra driver)
        self._prepare_statements()
        logger.info("✓ Checkpoint schema ready")

    async def _asetup_queryable_metadata(self) -> None:
        """
